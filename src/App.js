...
function App() {
  ...
  React.useEffect(() => {
    async function getData() {
      try {
        ...
        let resp = await axios.get('https://6385f366875ca3273d4b3960.mockapi.io/items');
    ...
    async function getFavoriteData() {
      try {
        let resp = await axios.get('https://6385f366875ca3273d4b3960.mockapi.io/favorite');
    ...
    axios
      .get('https://6385f366875ca3273d4b3960.mockapi.io/basket')
      .then((resp) => setCardItems(resp.data));

    axios
      .get('https://6385f366875ca3273d4b3960.mockapi.io/purchases')
    ...
    if (arrData.length > 0) {
      ...
      arrData.forEach((item) => {
        try {
          ...
          axios.post('https://6385f366875ca3273d4b3960.mockapi.io/purchases', item);
        } catch (err) {
  ...
  const handlerClickDeleteCard = async (path) => {
    try {
      let { data } = await axios.get('https://6385f366875ca3273d4b3960.mockapi.io/basket');

      data.map((item, index) => {
        if (item.product.includes(path)) {
          axios.delete(`https://6385f366875ca3273d4b3960.mockapi.io/basket/${item.id}`);
  ...
  const handlerClickDeleteFavorite = async (path) => {
    try {
      let { data } = await axios.get('https://6385f366875ca3273d4b3960.mockapi.io/favorite');

      data.map((item, index) => {
        if (item.product.includes(path)) {
          axios.delete(`https://6385f366875ca3273d4b3960.mockapi.io/favorite/${item.id}`);

          setFavoriteItems(
            favoriteItems.filter((arr, i) => favoriteItems[i] !== favoriteItems[index]),
          );
  ...
  const onAddToCart = (obj) => {
    try {
      ...
      axios.post('https://6385f366875ca3273d4b3960.mockapi.io/basket', obj);
    } catch (err) {
  ...
  const onAddToFavorite = (obj) => {
    try {
      ...
      axios.post('https://6385f366875ca3273d4b3960.mockapi.io/favorite', obj);
    } catch (err) {
  ...
  const onChangeSearch = (e) => {
    ...
    setSearchItems(
      productData.filter((item) => item.desc.toLowerCase().includes(e.target.value.toLowerCase())),
    );
  };
  ...
  return (
    <AppContext.Provider value={{ purchasesHistory, cardItems, total, setTotal, tax, setTax }}>
      <div className="wrapper">
        ...
        <header>
          <Header onClickBasket={handlerBasketClick} />
        </header>
        <main>
